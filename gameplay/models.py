...
class Game(models.Model):
    ...
    def new_move(self):
        ...
        if self.status not in "FS":
            raise ValueError("You can not make move on finished game")

        return Move(
            game=self,
            move_made_by_first_player=self.status == "F"
        )
    ...
    def _get_game_status_after_move(self, move):
        ...
        if (move == board[y][0] == board[y][1] == board[y][2]) or \
           ...
           (move == board[0][2] == board[1][1] == board[2][0]):
            return "W" if move.move_made_by_first_player else "L"
...
class Move(models.Model):
    ...
    move_made_by_first_player = models.BooleanField(editable=False)
    ...
    def __eq__(self, other):
        if other is None:
            return False
        return other.move_made_by_first_player == self.move_made_by_first_player
    ...
    def __str__(self):
        if self.move_made_by_first_player:
            return f"{self.x}, {self.y} move made by {self.game.first_player.username}"
        else:
            return f"{self.x}, {self.y} move made by {self.game.second_player.username}"
